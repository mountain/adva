...
@dataclass(frozen=True, slots=True)
class E0GridPoint:
    """One exact rational point of the upper-half-plane E0 chart."""
    ...
    def __post_init__(self) -> None:
        if self.y <= 0:
            raise ValueError("an E0 grid point must lie in the upper half-plane")

    @property
    def assignment(self) -> Fraction:
        return -self.x / self.y

    def add(self, step: Fraction) -> E0GridPoint:
        return E0GridPoint(self.x - step * self.y, self.y)

    def scale(self, factor: Fraction) -> E0GridPoint:
        if factor <= 0:
            raise ValueError("an E0 scale factor must be positive")
        return E0GridPoint(self.x, self.y / factor)

    def j(self) -> E0GridPoint:
        denominator = self.x * self.x + self.y * self.y
        return E0GridPoint(-self.x / denominator, self.y / denominator)


MobiusSupport: TypeAlias = frozenset[
    tuple[str, tuple[E0GridPoint, ...]]
]


@dataclass(frozen=True, slots=True)
class E0EmbeddedWire:
    """One checked program wire realized by exact X_1/Y_2 grid steps."""

    edge_id: str
    points: tuple[E0GridPoint, ...]

    def step_word(self) -> tuple[str, ...]:
        result = []
        for source, target in zip(self.points[:-1], self.points[1:], strict=True):
            if target == source.add(Fraction(1)):
                result.append("X")
            elif target == source.scale(Fraction(2)):
                result.append("Y")
            else:
                raise ValueError("wire path is not generated by X_1 and Y_2")
        return tuple(result)

    @property
    def mobius_path(self) -> tuple[E0GridPoint, ...]:
        return tuple(point.j() for point in self.points)


@dataclass(frozen=True, slots=True)
class E0MobiusCellularEdge:
    """The common wire index joining J(G) to the cellular dual G-dagger."""

    edge_id: str
    straight_path: tuple[E0GridPoint, ...]
    mobius_path: tuple[E0GridPoint, ...]
    cellular_dual_vertices: tuple[str, str]
...
class E0DiamondGridEmbedding:
    """Exact BS(2,1) rectilinear embedding of the checked causal diamond."""

    def __init__(self, model: DiamondDualPresentation) -> None:
        q = Fraction
        source = E0GridPoint(q(0), q(2))
        copy = E0GridPoint(q(0), q(1))
        neg = E0GridPoint(q(-1), q(1))
        identity = E0GridPoint(q(0), q(1, 2))
        midpoint = E0GridPoint(q(-1, 2), q(1, 2))
        add = E0GridPoint(q(-1), q(1, 2))
        target = E0GridPoint(q(-1), q(1, 4))
        ...
        self.boundary_points = {"input:0": source, "output:0": target}
        self.face_path = (copy, identity, midpoint, add, neg, copy)
        self.wires = {
            "node:0:0": E0EmbeddedWire("node:0:0", (source, copy)),
            "node:1:0": E0EmbeddedWire("node:1:0", (copy, neg)),
            "node:2:0": E0EmbeddedWire("node:2:0", (copy, identity)),
            "node:3:0": E0EmbeddedWire("node:3:0", (neg, add)),
            "node:3:1": E0EmbeddedWire(
                "node:3:1", (identity, midpoint, add)
            ),
            "output:0": E0EmbeddedWire("output:0", (add, target)),
        }
        self._validate()

    def _validate(self) -> None:
        if set(self.wires) != set(self.model.edge_by_id):
            raise ValueError("the E0 embedding does not cover every checked wire")
        producer_points = {
            "input:0": self.boundary_points["input:0"],
            **{
                f"node:{event}": point
                for event, point in self.event_points.items()
            },
        }
        consumer_points = {
            "node:0:0": self.event_points[0],
            "node:1:0": self.event_points[1],
            "node:2:0": self.event_points[2],
            "node:3:0": self.event_points[3],
            "node:3:1": self.event_points[3],
            "output:0": self.boundary_points["output:0"],
        }
        for edge in self.model.edges:
            wire = self.wires[edge.edge_id]
            if wire.points[0] != producer_points[edge.producer]:
                raise ValueError("the E0 wire does not start at its producer")
            if wire.points[-1] != consumer_points[edge.consumer]:
                raise ValueError("the E0 wire does not end at its consumer")
            wire.step_word()


class E0MobiusCellularBridge:
    """One edge-index span between J(G) and the cellular dual of G."""

    def __init__(self, embedding: E0DiamondGridEmbedding) -> None:
        self.embedding = embedding
        self.records = {
            edge_id: E0MobiusCellularEdge(
                edge_id=edge_id,
                straight_path=wire.points,
                mobius_path=wire.mobius_path,
                cellular_dual_vertices=(
                    embedding.model.edge_by_id[edge_id].dual_vertices
                ),
            )
            for edge_id, wire in embedding.wires.items()
        }

    def mobius_support(self, support: frozenset[str]) -> MobiusSupport:
        unknown = support - self.records.keys()
        if unknown:
            raise ValueError(f"unknown E0 edge identities: {sorted(unknown)}")
        return frozenset(
            (edge_id, self.records[edge_id].mobius_path)
            for edge_id in support
        )

    def cellular_support(self, support: MobiusSupport) -> frozenset[str]:
        result = set()
        for edge_id, path in support:
            record = self.records.get(edge_id)
            if record is None or record.mobius_path != path:
                raise ValueError("the Mobius path is not the J-image of its wire")
            result.add(edge_id)
        return frozenset(result)


def _dual_add(point: E0GridPoint, step: Fraction) -> E0GridPoint:
    return point.j().add(step).j()


def _dual_scale(point: E0GridPoint, factor: Fraction) -> E0GridPoint:
    return point.j().scale(factor).j()
...
def test_checked_diamond_has_an_exact_e0_bs21_grid_embedding() -> None:
    whole = _diamond()
    model = DiamondDualPresentation(whole)
    embedding = E0DiamondGridEmbedding(model)

    assert {
        edge_id: wire.step_word()
        for edge_id, wire in embedding.wires.items()
    } == {
        "node:0:0": ("Y",),
        "node:1:0": ("X",),
    ...
    copy = embedding.event_points[0]
    direct = copy.add(Fraction(1))
    around_cell = (
        copy.scale(Fraction(2))
        .add(Fraction(1))
        .add(Fraction(1))
        .scale(Fraction(1, 2))
    )
    assert around_cell == direct == embedding.event_points[1]
    assert embedding.face_path[0] == embedding.face_path[-1]
    assert embedding.face_path == (
        copy,
        copy.scale(Fraction(2)),
        copy.scale(Fraction(2)).add(Fraction(1)),
        copy.scale(Fraction(2)).add(Fraction(1)).add(Fraction(1)),
        direct,
        copy,
    )

    for wire in embedding.wires.values():
        for source, target, letter in zip(
            wire.points[:-1],
            wire.points[1:],
            wire.step_word(),
            strict=True,
            ...
            if letter == "X":
                assert target.assignment == source.assignment + 1
            else:
                assert target.assignment == 2 * source.assignment


def test_mobius_image_is_an_exact_curved_grid_with_bs12_polarity() -> None:
    model = DiamondDualPresentation(_diamond())
    embedding = E0DiamondGridEmbedding(model)
    points = frozenset(
        point
        for wire in embedding.wires.values()
    ...
    for point in points:
        assert point.j().j() == point
        assert point.j().assignment == -point.assignment
    ...
    for wire in embedding.wires.values():
        for source, target, letter in zip(
            ...
            if letter == "X":
                height = source.y
                center = Fraction(1, 2) / height
                radius_squared = Fraction(1, 4) / (height * height)
                for image in (source.j(), target.j()):
                    assert (
                        image.x * image.x
                        + (image.y - center) * (image.y - center)
                        == radius_squared
                    )
            else:
                horizontal = source.x
                for image in (source.j(), target.j()):
                    if horizontal == 0:
                        ...
                        center = Fraction(1, 2) / horizontal
                        radius_squared = Fraction(1, 4) / (
                            horizontal * horizontal
                        )
                        assert (
                            (image.x + center) * (image.x + center)
                            + image.y * image.y
                            == radius_squared
                        )

    point = embedding.event_points[0].j()
    b_then_x_then_b_inverse = _dual_scale(
        _dual_add(_dual_scale(point, Fraction(1, 2)), Fraction(1)),
        Fraction(2),
    )
    x_squared = _dual_add(_dual_add(point, Fraction(1)), Fraction(1))
    assert b_then_x_then_b_inverse == x_squared
    assert tuple(item.j() for item in embedding.face_path)[0] == tuple(
        item.j() for item in embedding.face_path
    )[-1]


def test_mobius_cellular_bridge_sends_every_checked_cut_to_a_dual_cycle() -> None:
    whole = _diamond()
    model = DiamondDualPresentation(whole)
    embedding = E0DiamondGridEmbedding(model)
    bridge = E0MobiusCellularBridge(embedding)

    assert set(bridge.records) == set(model.edge_by_id)
    for edge_id, record in bridge.records.items():
        assert record.straight_path == embedding.wires[edge_id].points
        assert record.mobius_path == tuple(
            point.j() for point in record.straight_path
        )
        assert record.cellular_dual_vertices == (
            model.edge_by_id[edge_id].dual_vertices
        )

    for completed in model.completed_pasts():
        state = model.encode_cut(whole.causal_cut(tuple(completed)))
        mobius_cycle = bridge.mobius_support(state.cycle)

        assert bridge.cellular_support(mobius_cycle) == state.cycle
        assert model.is_mod_two_cycle(bridge.cellular_support(mobius_cycle))


def test_mobius_cellular_bridge_intertwines_every_enabled_surgery() -> None:
    whole = _diamond()
    model = DiamondDualPresentation(whole)
    bridge = E0MobiusCellularBridge(E0DiamondGridEmbedding(model))
    ...
    for completed in model.completed_pasts():
        ...
        for event in model.enabled(completed):
            ...
            assert model.is_mod_two_cycle(boundary)
            assert bridge.cellular_support(
                bridge.mobius_support(boundary)
            ) == boundary
            assert bridge.mobius_support(after.cycle) == (
                bridge.mobius_support(before.cycle)
                ^ bridge.mobius_support(boundary)
            )


def test_mobius_cellular_bridge_is_not_a_full_cell_complex_isomorphism() -> None:
    model = DiamondDualPresentation(_diamond())
    embedding = E0DiamondGridEmbedding(model)
    geometric_vertices = {
        point
    ...
    cellular_dual_vertices = {
        ...
        for vertex in edge.dual_vertices
    }

    assert len(geometric_vertices) == 7
    assert cellular_dual_vertices == {model.OUTER_FACE, model.INNER_FACE}
    assert len(geometric_vertices) != len(cellular_dual_vertices)


def test_e0_routing_is_not_generic_multihole_arithmetic_evaluation() -> None:
    model = DiamondDualPresentation(_diamond())
    embedding = E0DiamondGridEmbedding(model)
    copy = embedding.event_points[0]
    neg = embedding.event_points[1]
    identity = embedding.event_points[2]
    add = embedding.event_points[3]

    assert neg.assignment != -copy.assignment
    assert add.assignment != neg.assignment + identity.assignment

    variable = Fraction(3, 5)
    assert variable + 1 != -variable
    assert 2 * variable != variable

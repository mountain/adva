...
ACTIVE = DIR / "contract-v10.json"
V9 = DIR / "contract-v9.json"
...
def test_the_chain_supersedes_by_digest_from_v0_through_the_active_contract():
    ...
    chain = [ACTIVE, V9, V8, V7, V6, V5, V4, V3, V2, V1, FROZEN]
    for path in chain:
    ...
    assert load(ACTIVE)["version"] == 10
    assert load(V9)["version"] == 9
...
def test_the_successors_only_moved_the_base_and_not_the_pins():
    """Successors v2-v10 keep the inputs; v4-v10 preserve every execution limit."""
    ...
    assert {k: v for k, v in load(ACTIVE).items() if k not in changed_metadata} == {
        k: v for k, v in load(V9).items() if k not in changed_metadata
    }
    assert {k: v for k, v in load(V9).items() if k not in changed_metadata} == {
...
def test_the_module_uses_the_successor_and_verifies_the_frozen_digest():
    ...
    assert 'CONTRACT = ROOT / "experiments/advance_symbol_surface/contract-v10.json"' in source

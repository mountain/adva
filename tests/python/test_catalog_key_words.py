...
def test_actual_catalog_v1_checks_all_fifteen_names():
...
def test_actual_catalog_growth_requires_sixteenth_name(tmp_path):
    ...
    report = catalog.check_catalog(root, key_words=True)
    assert report["status"] == "CatalogConsistent", report
    assert len(report["key_words"]["entries"]) == 16
    assert report["native_admission"] == "not-granted"

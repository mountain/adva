...
def test_actual_catalog_v1_checks_all_twenty_four_names():
...
def test_actual_catalog_growth_requires_twenty_fifth_name(tmp_path):
    ...
    report = catalog.check_catalog(root, key_words=True)
    assert report["status"] == "CatalogConsistent", report
    assert len(report["key_words"]["entries"]) == 25
    assert report["native_admission"] == "not-granted"

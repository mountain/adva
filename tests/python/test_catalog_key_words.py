...
def named_repository(repository):
    ...
    owner["materials"] = [source_ref, {"path": catalog.KEY_WORDS_PATH, "sha256": "0" * 64}]
...
def repin(root, document):
    ...
    next(r for r in owner["materials"] if r["path"] == catalog.KEY_WORDS_PATH)["sha256"] = (
...
def test_actual_catalog_v1_checks_all_fourteen_names():
...
@pytest.mark.parametrize("words", [[str(i) for i in range(17)], ["a" * 81], ["a" * 40, "b" * 40]])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda d: d["entries"].append({"key": "logic-foreign", "key_words": ["logic", "foreign"]}),
...
def test_unregistered_document_does_not_gain_admission(repository):
    ...
    assert catalog.check_catalog(repository, key_words=True)["status"] == "InvalidCatalog"


def test_new_instance_reuses_the_contract_without_changing_exception_list(named_repository):
    ...
    manifest = read_manifest(named_repository)
    manifest["entries"][-1]["title"] = "A different human title, not a proven interpretation"
    save_catalog(named_repository, manifest)
    assert catalog.check_catalog(named_repository, key_words=True)["status"] == "CatalogConsistent"
...
def test_word_budget_returns_unknown_at_catalog_boundary(named_repository):
    ...
    repin(named_repository, d)
    assert catalog.check_catalog(named_repository, key_words=True)["status"] == "Unknown"

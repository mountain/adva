...
def replay(probe, root, remaining):
    ...
    with tempfile.TemporaryFile() as out, tempfile.TemporaryFile() as err:
        p = subprocess.Popen([sys.executable, "-I", str(root / "scripts/defect_discovery/run.py"), "--worker", probe,
    ...
    receipt = dict(exit_code=p.returncode, timed_out=timeout,
...
def discover(root, out, source_commit):
    ...
    for name in SOURCES:
        ...
        report["probes"].append(dict(id=name, state=classify(receipts),
                                     upstream_state="NotEligible", replays=receipts))

...
PY_BODY = b"""import sys
n=lambda v:'['+','.join(map(str,v))+']'
sys.stdout.write('fn main(){let a:&[u8]=&'+n(a)+';let b:&[u8]=&'+n(b)+';let c:&[u8]=&'+n(c)+';'+bytes(b).decode('ascii')+'}\\n')
"""
RS_BODY = br'''let n=|v:&[u8]|format!("[{}]",v.iter().map(|x|x.to_string()).collect::<Vec<_>>().join(","));
let p=format!("a={}\nb={}\nc={}\n{}",n(a),n(b),n(c),std::str::from_utf8(a).unwrap());
let e=p.bytes().map(|x|{let u=i32::from(x%8)+1;format!("(add (add (copy {})) {})",u,i32::from(x)-2*u)}).collect::<Vec<_>>().join(" ");
println!("(module relay (export main) (def main (fn () (outputs {}) (frontier {}))))",vec!["Real";p.len()].join(" "),e);
'''
...
def python_source(context):
    return b"a=" + numbers(PY_BODY) + b"\nb=" + numbers(RS_BODY) + b"\nc=" + numbers(context) + b"\n" + PY_BODY


def expected_rust(context):
    return b"fn main(){let a:&[u8]=&" + numbers(PY_BODY) + b";let b:&[u8]=&" + numbers(RS_BODY) + b";let c:&[u8]=&" + numbers(context) + b";" + RS_BODY + b"}\n"


def inspect_python(source, context):
    """Check the complete generated source against the frozen no-source-read template."""
    tree = ast.parse(source)
    for statement, name, expected in zip(tree.body[:3], ("a", "b", "c"), (PY_BODY, RS_BODY, context), strict=True):
        if not isinstance(statement, ast.Assign) or len(statement.targets) != 1 or not isinstance(statement.targets[0], ast.Name) or statement.targets[0].id != name:
...
def check_context(context, contract):
    if context["embedded"]["commit"] != contract["library_commit"] or context["standalone"]["commit"] != contract["library_commit"]:
    ...
    if context["embedded"]["role"] != "gitlink-dependency" or context["standalone"]["role"] != "named-checkout":
...
class Supervisor:
    ...
    def restrictions(self):
        ...
        remaining = self.limits["aggregate_child_cpu_seconds"] - (self.child_cpu() - self.child_cpu_start)
    ...
    def call(self, name, command, cwd, *, output=None, allow_failure=False):
        ...
        with stdout.open("xb") as out, stderr.open("xb") as err:
            child = subprocess.Popen(record["argv"], cwd=cwd, env=env, stdin=subprocess.DEVNULL,
                                     stdout=out, stderr=err, start_new_session=True, preexec_fn=self.restrictions)
            try:
                ...
                try:
                    ...
                    pass
                child.wait()
        record.update(exit_code=child.returncode, wall_seconds=time.monotonic() - started,
        ...
        if child.returncode and not allow_failure:
            ...
            if b"File size limit exceeded" in diagnostic or b"memory allocation of" in diagnostic or b"out of memory" in diagnostic:
    ...
    def inventory(self):
        ...
        for path in sorted(self.output.rglob("*")):
            if path.is_file() and path.name != "report.json":
                ...
                files[str(path.relative_to(self.output))] = {"bytes": size, "sha256": digest(path.read_bytes())}
                self.check()
        return files, total
...
def run(args):
    ...
    if getattr(args, "previous", None):
        ...
        if prior.get("continuation") != "none-automatic" or prior.get("contract_sha256") != digest(contract_raw) or prior.get("status") == "ClosedFiniteByteRelay":
        ...
        if min(limits["wall_seconds"], limits["aggregate_child_cpu_seconds"], limits["max_processes"], limits["max_retained_bytes"]) <= 0:
            raise Exhausted("no correction budget remains")
        continuation = {"previous_report": str(previous), "previous_sha256": digest(previous_raw),
                        "correction": "Strip debug information at link time; consolidate read-only Git probes.",
                        "remaining_limits": limits, "prior_retained_bytes": retained,
                        "boundary_recheck": "Both library contexts and witnesses are rechecked again; no source, library, equality or output criterion changes."}
        (output / "continuation-contract.json").write_bytes(canonical(continuation) + b"\n")
    supervisor = Supervisor(output, limits)
    report = {"schema": "adva.quine-relay-report.research.v0", "status": "Unknown", "phase": "preflight",
              "controls": {}, "native_semantics_changed": False, "library_admission": "NotGranted",
              "general_duality": "Open", "continuation": "correction-used" if continuation else "none-automatic", "contract_sha256": digest(contract_raw)}
    constructed = 0
    try:
        implementation = output / "implementation"
        implementation.mkdir()
        for source in ("python/adva/quine_relay.py", "python/adva/adva.py", "crates/adva-witness/examples/quine_relay.rs", "Cargo.lock"):
            (implementation / source.replace("/", "__")).write_bytes((root / source).read_bytes())
        base, pin = supervisor.git("base-and-gitlink", root, "rev-parse", "HEAD", "HEAD:adva-library").splitlines()
        if base != contract["base_commit"]:
            raise ValueError("main checkout differs from the frozen source base")
        inside, inside_tree = supervisor.git("embedded-commit-tree", internal, "rev-parse", "HEAD", "HEAD^{tree}").splitlines()
        outside, outside_tree = supervisor.git("standalone-commit-tree", external, "rev-parse", "HEAD", "HEAD^{tree}").splitlines()
        ...
        origins = [supervisor.git(name, directory, "remote", "get-url", "origin") for name, directory in
                   (("embedded-origin", internal), ("standalone-origin", external))]
        if origins[0] != origins[1]:
            raise ValueError("library remotes differ")
        pins = {}
        for name in ("stability/epoch-0000.json", "stability/epoch-0001.json", "math/manifest.json",
                     "math/constraints/growth-obligation-v0000.json", "math/constraints/growth-obligation-seal-v0000.json"):
            a, b = (internal / name).read_bytes(), (external / name).read_bytes()
            if a != b:
                raise ValueError("library material bytes differ: " + name)
            pins[name] = digest(a)
        context = {"schema": "quine-library-context-v0", "repository": origins[0], "tree": inside_tree,
                   "embedded": {"role": "gitlink-dependency", "path": str(internal), "commit": pin},
                   "standalone": {"role": "named-checkout", "path": str(external), "commit": outside, "branch": branch},
                   "entry_views": {"p": "standalone", "q": "embedded", "r": "byte-observation"}, "pins": pins}
        ...
        supervisor.call("build-observer", ["cargo", "rustc", "--locked", "--offline", "-j", "1", "-p", "adva-witness", "--example", "quine_relay", "--", "-C", "debuginfo=0", "-C", "strip=debuginfo"], root)
        native = root / "target/debug/examples/quine_relay"
        supervisor.call("check-libraries", [native, "libraries", internal, external, output / "library-recheck.json"], root)
        library = json.loads((output / "library-recheck.json").read_bytes())
        report["controls"]["zero_guard_rejected_in_both_libraries"] = all(x["zero_refusal"] for x in library["readings"])
        ...
        supervisor.call("p-to-q", [sys.executable, "-I", "-S", output / "p.py"], empty, output=output / "q.rs")
        q = (output / "q.rs").read_bytes()
        if q != expected_rust(encoded):
            raise ValueError("Rust source differs from frozen mutual quotation")
        supervisor.call("compile-q", ["rustc", "--edition=2024", output / "q.rs", "-o", output / "q-bin"], empty)
        ...
        supervisor.call("r-to-p", [native, "emit", output / "r.adva", output / "p-regenerated.py", output / "r-native.json"], empty)
        ...
        report["closure"] = {"p_bytes": len(p), "q_bytes": len(q), "r_bytes": len(r),
                             "p_sha256": digest(p), "regenerated_sha256": digest(regenerated), "byte_equal": True}
        ...
        supervisor.call("regenerated-p-to-q", [sys.executable, "-I", "-S", output / "p-regenerated.py"], empty, output=output / "q-regenerated.rs")
        report["controls"]["regenerated_p_repeats_q"] = (output / "q-regenerated.rs").read_bytes() == q
        for name, body, reason in (("fraction", "1/2", "finite integers"), ("unknown-print", "(print 65)", "unknown operation")):
            path = output / (name + ".adva")
            path.write_text("(module relay (export main) (def main (fn () Real " + body + ")))\n")
            result_path = output / (name + ".json")
            exit_code = supervisor.call(name, [native, "emit", path, output / (name + ".out"), result_path], empty, allow_failure=True)
            refusal = json.loads(result_path.read_bytes())
            report["controls"][name + "_rejected"] = exit_code != 0 and reason in refusal.get("error", "") and not (output / (name + ".out")).exists()
        ...
        supervisor.call("scale-control", [native, "emit", output / "r-scale-control.adva", output / "p-scale-control.py", output / "scale-native.json"], empty)
        additive = json.loads((output / "r-native.json").read_bytes())
        multiplicative = json.loads((output / "scale-native.json").read_bytes())
        report["controls"]["same_bytes_distinct_diagrams"] = (output / "p-scale-control.py").read_bytes() == p and additive["compilation"]["result"] != multiplicative["compilation"]["result"]
        report["native"] = {key: additive[key] for key in ("ast_terms", "compiled_nodes", "source_count", "output_bytes")}
        ...
        for name, expected in pins.items():
            if any(digest((directory / name).read_bytes()) != expected for directory in (internal, external)):
        ...
        report["cost"] = {"wall_seconds_before_report": time.monotonic() - supervisor.started,
                          "child_cpu_seconds": supervisor.child_cpu() - supervisor.child_cpu_start,
                          "processes": len(supervisor.calls), "candidate_constructions": constructed,

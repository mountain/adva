...
class Supervisor:
    ...
    def restrictions(self):
        ...
        resource.setrlimit(resource.RLIMIT_CPU, (self.limits["child_cpu_seconds"],) * 2)
...
def run(args):
    ...
    report = {"schema": "adva.quine-relay-report.research.v0", "status": "Unknown", "phase": "preflight",
              ...
              "general_duality": "Open", "continuation": "none-automatic", "contract_sha256": digest(contract_raw)}
    try:
        base = supervisor.git("base", root, "rev-parse", "HEAD")
        if base != contract["base_commit"]:
            raise ValueError("main checkout differs from the frozen source base")
        pin = supervisor.git("gitlink", root, "rev-parse", "HEAD:adva-library")
        inside = supervisor.git("embedded-head", internal, "rev-parse", "HEAD")
        outside = supervisor.git("standalone-head", external, "rev-parse", "HEAD")
        inside_tree = supervisor.git("embedded-tree", internal, "rev-parse", "HEAD^{tree}")
        outside_tree = supervisor.git("standalone-tree", external, "rev-parse", "HEAD^{tree}")
        ...
        supervisor.call("build-observer", ["cargo", "build", "--locked", "--offline", "-p", "adva-witness", "--example", "quine_relay"], root)
        ...
        report["cost"] = {"wall_seconds_before_report": time.monotonic() - supervisor.started,
                          ...
                          "processes": len(supervisor.calls), "candidate_constructions": 1,

...
theorem adva152e1 (x : Int) : (((2 : Int) * x) + ((2 : Int) * x)) = (((2 : Int) * x) + (x + x)) := (congrArg (fun (z : Int) => (((2 : Int) * x) + z)) ((Int.two_mul x)))
#print axioms adva152e1
theorem adva152e2 (x : Int) : (((2 : Int) * x) + (x + x)) = ((x + x) + (x + x)) := (congrArg (fun (z : Int) => (z + (x + x))) ((Int.two_mul x)))

...
def unpack_export(raw, mode):
    ...
    if len(set(labels)) != len(labels) or any(not re.fullmatch(r"adva152(?:e|cal)[0-9]+", label) for label in labels):
...
class Supervisor:
    ...
    def call(self, name, command, input_text=None):
        ...
        with (self.output / (name + ".stdout")).open("xb") as stdout, (self.output / (name + ".stderr")).open("xb") as stderr:
            ...
            timeout = False
            try:
                process.communicate(None if input_text is None else input_text.encode(), timeout=min(180, remaining))
                ...
                entry.update(returncode=process.returncode, wall_seconds=time.monotonic() - start,
...
def run(args):
    ...
    report = {"schema": "adva.three-verifier-run.research.v0", "status": "Blocked",
              "rounds": [], "calls": supervisor.calls, "allowed_action": None}
    try:
        ...
        report["tools"] = {"native": {"path": native, "sha256": digest(native)},
        ...
        report["implementation_sha256"] = {str(p.relative_to(root)): digest(p) for p in (
            Path(__file__), Path(__file__).with_name("adva.py"),
        ...
        rc, raw, err = supervisor.call("metamath-base", [mm], f'READ "{database}"\nVERIFY PROOF *\nEXIT\n')
        ...
        for mode in ("calibrate", "2", "3", "4"):
            ...
            lean_file = supervisor.save(f"proofs-{mode}.lean", "import Init\n" + "".join(edge["lean"] for edge in edges))
            mm_file = supervisor.save(f"proofs-{mode}.mm", f"$[ {database} $]\n" + "".join(edge["metamath"] for edge in edges))
            ...
            rc, log, err = supervisor.call(f"metamath-{mode}", [mm], f'READ "{mm_file}"\nVERIFY PROOF adva152*\nEXIT\n')
            if rc or err:
            ...
            if mode == "calibrate":
                ...
                rc, log, err = supervisor.call("metamath-negative", [mm], f'READ "{bad_mm}"\nVERIFY PROOF adva152bad\nEXIT\n')
                ...
                report["calibration"] = {"status": "Passed", "controls": obj["result"]["controls"],
                                         "external_controls": ["LeanFalseRejected", "LeanSorryRejected", "MetamathFalseRejected"]}
            else:
                arms = [{k: arm[k] for k in ("depth", "expand", "seed", "policy", "status", "steps", "residual", "stop_reason")}
                        for arm in obj["result"]["arms"]]
                report["rounds"].append({"depth": int(mode), "status": "ThreeVerifierChecked",
                                         "counts": obj["counts"], "arms": arms})
            supervisor.save(f"admission-{mode}.json", json.dumps({"status": "ThreeVerifierChecked", "labels": labels,
                            "axioms": axioms, "export_sha256": hashlib.sha256(raw).hexdigest()}, indent=2) + "\n")
            print(json.dumps({"mode": mode, "status": "ThreeVerifierChecked", "edges": len(edges)}), flush=True)
        if digest(database) != DATABASE_SHA256:
            raise ValueError("database changed during the run")
        report.update(status="Completed", allowed_action="retain-scoped-calibration-and-search-evidence",
                      aggregate={"enumerated_including_replay": total_candidates,
        ...
        report["cost"] = {"wall_seconds_before_final_checkpoint": time.monotonic() - supervisor.started,

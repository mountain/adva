...
This phase added no `.adva` program to the repository and its tests still use
only temporary files. The later first program in
`programs/bootstrap-0/reveal.adva` is an explicit ADR 0023 calibration rather
than an artifact silently generated by this codec.

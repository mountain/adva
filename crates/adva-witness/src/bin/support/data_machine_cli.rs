...
use adva_witness::{
    DATA_MACHINE_MAX_BYTES_V0, DATA_MACHINE_MAX_REPORT_V0, DataMachineProgramV0, DataMachineRunV0,
    DataMachineStatusV0, MachineDataV0, resume_data_machine_v0, run_data_machine_v0,
    verify_data_machine_run_v0,
};
...
pub fn run(mut arguments: impl Iterator<Item = String>) -> Result<(), Box<dyn Error>> {
    ...
    let program: DataMachineProgramV0 =
        serde_json::from_str(&read(&program_path, DATA_MACHINE_MAX_BYTES_V0)?)?;
    let input: MachineDataV0 =
        serde_json::from_str(&read(required("--input")?, DATA_MACHINE_MAX_BYTES_V0)?)?;
    let (bytes, rejected, status) = if let Some(path) = options.get("--check") {
        if quantum != 0 {
            return Err(invalid("check requires zero quantum").into());
        }
        let checkpoint: DataMachineRunV0 =
            serde_json::from_str(&read(path, DATA_MACHINE_MAX_REPORT_V0)?)?;
        let reception =
            verify_data_machine_run_v0(&program, &input, fuel, &checkpoint).map_err(invalid)?;
        (
        ...
        let report = if let Some(path) = options.get("--resume") {
            let checkpoint: DataMachineRunV0 =
                serde_json::from_str(&read(path, DATA_MACHINE_MAX_REPORT_V0)?)?;
            resume_data_machine_v0(&program, &input, fuel, &checkpoint, quantum)
        } else {
            run_data_machine_v0(&program, &input, fuel, quantum)
        }
        .map_err(invalid)?;
        (
            serde_json::to_vec_pretty(&report)?,
            report.status == DataMachineStatusV0::Rejected,
            format!("{:?}", report.status),
        )
    };
    if bytes.len() > DATA_MACHINE_MAX_REPORT_V0 {

...
use adva_witness::{
    DATA_MACHINE_MAX_BYTES_V2, DATA_MACHINE_MAX_REPORT_V2, DataMachineProgramV2, DataMachineRunV2,
    DataMachineStatusV2, MachineDataV2, resume_data_machine_v2, run_data_machine_v2,
    verify_data_machine_run_v2,
};
...
pub fn run(mut arguments: impl Iterator<Item = String>) -> Result<(), Box<dyn Error>> {
    ...
    let program: DataMachineProgramV2 =
        serde_json::from_str(&read(&program_path, DATA_MACHINE_MAX_BYTES_V2)?)?;
    let input: MachineDataV2 =
        serde_json::from_str(&read(required("--input")?, DATA_MACHINE_MAX_BYTES_V2)?)?;
    let (bytes, rejected, status) = if let Some(path) = options.get("--check") {
        if quantum != 0 {
            return Err(invalid("check requires zero quantum").into());
        }
        let checkpoint: DataMachineRunV2 =
            serde_json::from_str(&read(path, DATA_MACHINE_MAX_REPORT_V2)?)?;
        let reception =
            verify_data_machine_run_v2(&program, &input, fuel, &checkpoint).map_err(invalid)?;
        (
        ...
        let report = if let Some(path) = options.get("--resume") {
            let checkpoint: DataMachineRunV2 =
                serde_json::from_str(&read(path, DATA_MACHINE_MAX_REPORT_V2)?)?;
            resume_data_machine_v2(&program, &input, fuel, &checkpoint, quantum)
        } else {
            run_data_machine_v2(&program, &input, fuel, quantum)
        }
        .map_err(invalid)?;
        (
            serde_json::to_vec_pretty(&report)?,
            report.status == DataMachineStatusV2::Rejected,
            format!("{:?}", report.status),
        )
    };
    if bytes.len() > DATA_MACHINE_MAX_REPORT_V2 {

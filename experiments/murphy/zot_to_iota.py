"""Translate a traced Zot value to pure Iota; check equality by beta-normalization.

Authored by ChatGPT (OpenAI). Standard library only. No eta rule or shortest-code claim.
Usage: python3 zot_to_iota.py TRACE_JSON OUTPUT_DIRECTORY
"""
...
def tick():
    ...
    if work>4_000_000 or time.monotonic()-started>30:
        raise RuntimeError('Verification budget exceeded; no equality claim')
V=lambda n:('v',n)
A=lambda f,x:('a',f,x)
L=lambda n,t:('l',n,t)
C=lambda n:('c',n)
...
term=closure(trace['programTree'])
encoded=encode(to_iota(ski(term)))
decoded=decode(encoded)
original_nf=normal(db(term));converted_nf=normal(db(decoded))
assert original_nf==converted_nf
for name,t in [('I',iota_i),('K',iota_k),('S',iota_s)]:
    assert normal(db(t))==normal(definitions[name])
# Check the return value as a separate observation, using a free opaque printer.
stop=C('I')
for _ in range(6):stop=A(C('K'),stop)
returned=closure(trace['returnedTree'])
wrapped=L('printer',apps(term,stop,V('printer')))
assert normal(db(wrapped))==normal(db(L('printer',returned)))
returned_code=encode(to_iota(ski(returned)))
assert normal(db(decode(returned_code)))==normal(db(returned))
(out/'zot_0001011011_value.iota').write_text(encoded+'\n')
evidence={
 'schema':'zot.iota.translation.v1','sourceWord':trace['word'],
 'target':'value after all ten bits, before stop/printer wrapper',
 'grammar':'i = iota; *AB = application(A,B); no implicit suffix',
 'iotaDefinition':'iota f = f S K',
 'method':'S/K/I bracket abstraction without eta; expand I,K,S to Iota',
 'iotaLeaves':encoded.count('i'),'applications':encoded.count('*'),'characters':len(encoded),
 'normalForm':pretty(original_nf),'betaNormalFormsEqual':True,
 'fullConsumptionRoundTrip':encode(decoded)==encoded,
 'returnedValueIota':returned_code,'wrappedReturnIgnoresPrinter':True,
 'countedVerificationWork':work,
 'limits':{'maxWork':4000000,'wallSeconds':30},
 'limitations':['No shortest-Iota claim','Pure lambda beta equivalence, not cost preservation','No general CBV/effect equivalence theorem'],
}
(out/'zot_0001011011_iota_evidence.json').write_text(json.dumps(evidence,ensure_ascii=False,indent=2)+'\n')

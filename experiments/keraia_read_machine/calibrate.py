...
def run():
    ...
    try:
        ...
        # Retain the four post-run failures from attempt-1 as refusal controls.
        identity_state = M.advance(M.State.start(identity, 128, budget), engine)
        type_refusals = []
        for field, value in (("steps", True), ("cursor", 5.0)):
            record = identity_state.record()
            record[field] = value
            try:
                M.restore_checkpoint(json.dumps(record), identity, 128, engine)
            except ValueError:
                type_refusals.append("checkpoint " + field)
        frame = M.Frame(I, (), 1)
        sample = engine.segment(frame, 128)
        for field, altered in (("cursor", replace(sample, end=replace(sample.end, cursor=True))),
                               ("variable_index", replace(sample, end=replace(sample.end, control=("l", ("v", 0.0)))))):
            if not M.check_receipt(altered, frame, budget, engine.counts):
                type_refusals.append("receipt " + field)
        check(len(type_refusals) == 4, "numeric coercion cannot cross the evidence boundary")
        evidence["type_refusals"] = type_refusals
        passed("all four retained bool/float boundary failures are now refused")

"""Finite mix trial accounting, adapted from the frozen self-compiler supervisor."""
import hashlib
import json
import os
...
from experiments.bounded_self_compiler.language import canonical
...
def profile():
    h=blake3.blake3(b'adva.data-machine.transition.v2\0')
    h.update((ROOT/'crates/adva-witness/src/data_machine_v2.rs').read_bytes())
...
class Trial:
    def __init__(self, binary, out, *, preflight=False):
        ...
        self.wall=180 if preflight else 1200
        self.cpu=160 if preflight else 1080
        self.maximum=96*1024**2 if preflight else 256*1024**2
        self.start=time.monotonic(); self.calls=[]; self.bytes=0
        self.max_calls=6 if preflight else 48
    ...
    def native(self,name,program,data,*,fuel=200000,quantum=None,resume=None,check=None,refuse=False):
        if len(self.calls)>=self.max_calls: raise RuntimeError('native launch budget')
        p=self.save(name+'.program.adva',program); d=self.save(name+'.input.json',data)
        output=self.out/(name+'.run.adva')
        cmd=[str(self.binary),'data-run-v2',str(p),'--input',str(d),'--fuel',str(fuel),
        ...
        if refuse:
            ...
            assert report['profile']==profile(),(name,'binary/source profile mismatch')
            assert result.returncode==(2 if report.get('status')=='Rejected' else 0),(name,result.stderr)
        return report

...
# How many powers of the pure-translation witness are computed. The witness is a
# translation by a nonzero lattice vector, so its nth power is the translation by
# the nth multiple; the powers checked are distinct and none is the identity, and
# the induction over the exact composition rule covers every remaining power.
INFINITE_ORDER_POWERS = 12
...
def s1_two_symmetry_sets():
    ...
    # A bounded count of 5781 elements through length twenty only says the closure is
    # LARGE. What makes it infinite is an element of infinite order, and the closure
    # contains one within the declared depth: a pure translation, an element whose
    # linear part is the identity whose translation vector is a nonzero lattice
    # vector. Its powers are the translations by the successive multiples of that
    # vector, so no positive power is the identity and the generated group is
    # infinite rather than merely uncounted.
    translations = sorted(
        (A for A in seen if A[0] == IDENTITY_PERM and A[1] != (0, 0, 0, 0)),
        key=lambda A: (seen[A], A[1]))
    check(bool(translations),
          "the closure contains no pure translation within the declared depth")
    witness = translations[0]
    witness_vector = witness[1]
    witness_length = seen[witness]
    powers = []
    for _ in range(INFINITE_ORDER_POWERS):
        powers.append(affine_compose(witness, powers[-1] if powers else AFFINE_IDENTITY))
    powers_are_the_multiples = all(
        A[0] == IDENTITY_PERM and A[1] == tuple(n * x for x in witness_vector)
        for n, A in enumerate(powers, start=1))
    powers_are_distinct = len(set(powers)) == INFINITE_ORDER_POWERS
    identity_is_not_a_power = AFFINE_IDENTITY not in powers
    check(powers_are_the_multiples,
          "a power of the pure translation is not the translation by the multiple of its vector")
    check(powers_are_distinct and identity_is_not_a_power,
          "two powers of the pure translation coincide or one of them is the identity")
    check(affine_compose(witness, witness)
          == (IDENTITY_PERM, tuple(2 * x for x in witness_vector)),
          "the one-step composition rule of the pure translation is not the declared one")
    # the composition rule (1,t)(1,u) = (1,t+u) is exact for every translation, so
    # (1,t)^n = (1,nt) by induction, and (1,nt) is the identity only if nt = 0
    infinite_closure = (bool(translations) and powers_are_the_multiples
                        and powers_are_distinct and identity_is_not_a_power)
    check(infinite_closure, "the closure is not shown to contain an element of infinite order")
    ...
    return {
        ...
        "closure_is_infinite": infinite_closure,
        "closure_is_infinite_reason":
            "the closure contains a pure translation inside the declared depth: an element "
            "whose linear part is the identity and whose translation vector is a nonzero "
            "lattice vector, so no positive power of it is the identity",
        "pure_translation_witness_vector": list(witness_vector),
        "pure_translation_witness_coxeter_length": witness_length,
        "pure_translations_within_the_declared_depth": len(translations),
        "pure_translation_powers_checked": INFINITE_ORDER_POWERS,
        "pure_translation_powers_are_the_translations_by_the_multiples":
            powers_are_the_multiples,
        "pure_translation_has_no_power_equal_to_the_identity": identity_is_not_a_power,
...
def null_law_by_weighted_arrangements(edges):
    """The same exact law, counted over distinct arrangements of the recipients.

    Permuting two equal recipients leaves the shuffled edge family unchanged, so the
    m! index permutations collapse onto the distinct arrangements of the recipient
    sequence, and each arrangement carries the number of index permutations that
    produces it, the product of the factorials of the recipient multiplicities. The
    law is the same object; s3_exact_null compares the two computations on the five
    declared networks before this one is used.
    """
    m = len(edges)
    agents = [a for a, _ in edges]
    recipients = [b for _, b in edges]
    weight = 1
    for count in Counter(recipients).values():
        weight *= _factorial(count)
    law = {}
    for arrangement in set(permutations(recipients)):
        value = reciprocity(list(zip(agents, arrangement)))
        law[value] = law.get(value, 0) + weight
    return law


# The declared sweep for the margins-only claim: every directed edge family on two
# or three labelled vertices with at most six edges. Self-loops are admitted, so the
# sweep is wider than the networks the contract declares.
SWEEP_VERTICES = (2, 3)
SWEEP_MAX_EDGES = 6


def swept_families(n_vertices, n_edges):
    pairs = [(a, b) for a in range(n_vertices) for b in range(n_vertices)]
    for family in combinations_with_replacement(pairs, n_edges):
        yield list(family)


def margin_class_sweep():
    """Check the law against the two margins alone over the whole declared sweep.

    Two families have the same margins when they have the same multiset of agents and
    the same multiset of recipients, edge for edge irrelevant. Every family of the
    sweep is enumerated, its exact law is computed, and the law is compared with the
    law of the first family of its own margin class. The class of a family is read off
    the margins only, before any law is computed, so agreeing laws are evidence for
    the claim and not for the way the class was built. Because a class of m edges on n
    vertices is a pair of multisets, a complete sweep must see exactly
    C(n+m-1, m)^2 classes per slice, which is asserted per slice below.
    """
    families, comparisons, violations = 0, 0, 0
    arrangements, index_permutations = 0, 0
    per_slice = {}
    all_keys = set()
    for n_vertices in SWEEP_VERTICES:
        for n_edges in range(SWEEP_MAX_EDGES + 1):
            first, seen_families = {}, 0
            for edges in swept_families(n_vertices, n_edges):
                families += 1
                seen_families += 1
                key = margins(edges)
                arrangements += len(set(permutations([b for _, b in edges])))
                index_permutations += _factorial(n_edges)
                law = null_law_by_weighted_arrangements(edges)
                if key in first:
                    comparisons += 1
                    violations += first[key] != law
                else:
                    first[key] = law
            # completeness of the slice, from the counting alone: a margin class of m
            # edges on n labelled vertices is a pair of multisets of size m
            check(len(first) == comb(n_vertices + n_edges - 1, n_edges) ** 2,
                  "the declared sweep did not reach every margin class of its slice")
            check(seen_families == comb(n_vertices ** 2 + n_edges - 1, n_edges),
                  "the declared sweep did not enumerate every family of its slice")
            all_keys |= set(first)
            per_slice[f"{n_vertices}_vertices_{n_edges}_edges"] = {
                "families": seen_families,
                "margin_classes": len(first),
            }
    slice_classes = sum(row["margin_classes"] for row in per_slice.values())
    check(slice_classes > 0, "the declared sweep found no margin class")
    check(comparisons > 0 and families > slice_classes,
          "the declared sweep exhibits no two families that share their margins")
    return {
        "violations": violations,
        "families_exhausted": families,
        "margin_classes_per_slice_summed": slice_classes,
        "distinct_margin_classes_across_the_slices": len(all_keys),
        "within_class_law_comparisons": comparisons,
        "recipient_arrangements_enumerated": arrangements,
        "index_permutations_they_stand_for": index_permutations,
        "vertices": list(SWEEP_VERTICES),
        "max_edges": SWEEP_MAX_EDGES,
        "self_loops_admitted": True,
        "per_slice": per_slice,
    }
...
def s3_exact_null():
    ...
    # the two implementations of the exact law agree on every declared network, so the
    # weighted arrangement sweep below computes the same object as the index enumeration
    for edges in families:
        check(null_law_by_weighted_arrangements(edges) == null_law(edges),
              "the weighted arrangement enumeration does not reproduce the permutation law")

    # the claim itself, over a declared sweep far wider than the five networks above:
    # every family on two or three vertices with at most six edges, compared inside its
    # own margin class. The boolean below is read off that exhaustion and not asserted.
    sweep = margin_class_sweep()
    check(sweep["violations"] == 0,
          "the null law is not a function of the two margins alone: a family of the "
          "declared sweep disagrees with another family of its own margin class")
    margins_only_law = sweep["violations"] == 0
    ...
    return {
        ...
        "null_law_depends_only_on_the_two_margins": margins_only_law,
        "null_law_margin_class_sweep": sweep,
        "null_law_margin_class_sweep_violations": sweep["violations"],

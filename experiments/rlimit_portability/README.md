...
Every count, table row and digest in this section describes that baseline snapshot
and is not rewritten. The audit scans the current tree, so its pin has to track the
tree: the deliberate, visible re-pin of 2026-09-24 is recorded at the end of this
file.
...
## 2026-09-24 re-pin after the audited tree grew

This audit scans the tree, so its pin has to follow the tree, and re-pinning is a
deliberate, visible commit rather than a silent refresh. Between the baseline and
commit `7a57d8d` the repository gained twelve Python files containing
`RLIMIT_AS`, and the pin went stale:
`tests/python/test_rlimit_portability.py::test_inventory_and_pins` failed with
**112 scanned files against the pinned 100**. `pytest` runs in CI
(`.github/workflows/ci.yml`), so this check is what keeps the pin honest; the
failure is repaired here by reviewing the added files, not by loosening the check.

Regenerated by the audit itself, from the repository root:

```sh
python experiments/rlimit_portability/audit.py > experiments/rlimit_portability/inventory.json
```

- `inventory.json` digest
  `a5e13d4b684a94c1625cef94434170530eaca4e369614d7da46690cd69064ced`
  (was `14fb463beca3338bc089ee489042d2498ff2d30128179910305625b7311a4bca`).
- State at the re-pin: **112 files and 114 AST references** (was 100 and 102), of
  which 15 historical copies; **83 unguarded installations** (was 71). The other
  categories are unchanged: 7 linux-required entries, 3 linux-only
  installations, 3 refusal-recorded, 1 mock-only.
- The twelve added files, each classified `unguarded-installation`, with the
  commit that introduced it and its AST reference lines:

| File | Commit | Lines |
| --- | --- | --- |
| `experiments/taixuan_hierarchy_spectrum_v1/check.py` | `45e374f` | 292 |
| `experiments/taixuan_spectral_analysis_v1/analyze.py` | `2fbeb30` | 264 |
| `experiments/taixuan_spectral_analysis_v1/bounded.py` | `2fbeb30` | 34 |
| `experiments/native_background_forecast_v1/run.py` | `610eb6c` | 23 |
| `experiments/spectral_memory_forecast_v1/run.py` | `610eb6c` | 23 |
| `experiments/japan_geostrophic_audit_v1/reproduce_full.py` | `f764683` | 14 |
| `experiments/japan_geostrophic_audit_v1/run.py` | `f764683` | 11 |
| `experiments/japan_geostrophic_audit_v1/verify_summary.py` | `f764683` | 29 |
| `experiments/floquet_forecast_audit_v1/attempt-1/run.py` | `19ac70e` | 10 |
| `experiments/floquet_forecast_audit_v1/run.py` | `19ac70e` | 10 |
| `experiments/floquet_forecast_audit_v1/verify.py` | `19ac70e` | 10 |
| `experiments/native_floquet_expression_v1/check.py` | `7a57d8d` | 59 |

- Nine of the twelve install AS only in an explicit `--worker` child branch, so a
  refusing API is contained in a child whose exit the parent classifies. Three
  install it unconditionally at entry — `floquet_forecast_audit_v1/verify.py`,
  `japan_geostrophic_audit_v1/verify_summary.py` and
  `native_floquet_expression_v1/check.py` — and there the refusal aborts the
  verifier or checker directly. `audit.py` now records those three, the
  spent-ledger branch of `reproduce_full.py` (the largest declared budget in this
  inventory: 540 s CPU, 6 GiB AS, 150 MB output) and the retained attempt below in
  its per-file `NOTES`.
- Retained open question, not resolved by this re-pin:
  `experiments/floquet_forecast_audit_v1/attempt-1/run.py` is byte identical to
  the current `experiments/floquet_forecast_audit_v1/run.py` and is a retained
  superseded attempt, but the frozen rule keys on `evidence/`, `run-01/` and
  `before-` names, so this copy is counted as a live installation. Whether a
  retained attempt directory should count as historical is a classification
  decision that this re-pin does not make.
- Scope of the change: `audit.py`, `inventory.json` and this README only. No other
  tracked file changed, no platform guard was added, weakened or removed, no
  evidence was rewritten, and no Darwin execution is claimed. `verification.json`

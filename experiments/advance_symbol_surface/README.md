...
[contract-v7.json](contract-v7.json). Each successor names the digest of the one
it supersedes, so the chain is `contract.json` (v0) <- `contract-v1.json` <-
`contract-v2.json` <- `contract-v3.json` <- `contract-v4.json` <- `contract-v5.json` <- `contract-v6.json` <- `contract-v7.json`, and a run verifies that digest rather than
trusting the file:
...
of v6 and starting no new run. The strict Rust-diff check remains unchanged.

...
SOURCES = [
 ...
 'programs/bounded-self-compiler/compiler.input.json','experiments/bounded_self_compiler/contract.json',
] + ['experiments/bounded_self_compiler/'+name+'.py' for name in
     ['author','language','checker','reference','fixtures','runtime','preflight','preflight_controls','campaign','supervise']]
...
def main():
    ...
    result={'status':'Failed','processes':[],'binary_sha256':sha(a.binary),
            'contract_sha256':manifest['experiments/bounded_self_compiler/contract.json']}
